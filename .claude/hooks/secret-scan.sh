...
# Kill-switch: .repo-meta/secret-scan.disabled   ·   También CLI: bash .claude/hooks/secret-scan.sh
#
...
[ -f .repo-meta/secret-scan.disabled ] && exit 0
...
if [ "$violations" -gt 0 ]; then
  ...
  if [ -n "$RANGE_BASE" ]; then
    ...
    echo "o a un gestor de secretos, y confirmá que .gitignore lo cubra." >&2
    echo "(Falso positivo puntual: git commit --no-verify, o kill-switch .repo-meta/secret-scan.disabled)" >&2
  fi
  exit 1
fi
exit 0
