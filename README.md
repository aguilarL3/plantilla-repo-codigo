...
| `search-sessions.py` | **Buscador de sesiones pasadas** — para el *cómo* que la bitácora no guarda: el error exacto, el comando que falló, lo que se probó antes. Lee `~/.claude/projects/<repo>/`, donde el harness ya guarda **todas** las sesiones solo: **funciona sin configurar nada y sin ningún hook**. Por eso este template **no trae `pre-compact.sh`** — sería duplicar en el repo (2–3 MB por sesión) algo que ya está en disco, y encima solo la parte que compactó. `--list` = inventario · `--role user` · `--regex` · `--full`. ⚠️ Solo ve **tus** sesiones en **esta** máquina: no es memoria de equipo. |
